...
function main() {
  ...
  for (const dir of skillDirs) {
    ...
    // WooCommerce skills should target WooCommerce 10.x+ (WP 6.7+, PHP 8.0+)
    assert(
      compatibility.includes("WooCommerce 10") && compatibility.includes("PHP 8.0"),
      `Compatibility contract mismatch in ${path.relative(repoRoot, skillPath)} (expected WooCommerce 10.x + PHP 8.0+)`
    );
  }
